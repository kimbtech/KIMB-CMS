...
else if(id=="#freig://list"){set_vars('frei');$("#firstbutt input[type=radio]#frei").attr("checked","checked")
$("div#firstbutt").buttonset("refresh")}
else{set_vars('my')}}}
...
else if(event.keyCode==17){$("div.toolbar span").css('display','none')}})});$("button#add_file").unbind('click').click(function(){var html='<h2>Dateien hochladen</h2>';html+='<form action="'+add_daten.siteurl+'/ajax.php?addon=daten" class="dropzone" id="dropzone">';html+='<input type="hidden" name="allgvars[folder]" value="'+allgvars.folder+'">';html+='<input type="hidden" name="allgvars[path]" value="'+allgvars.path+'">';html+='<input type="hidden" name="todo" value="uploadfile">';html+='<img src="'+add_daten.siteurl+'/load/addondata/daten/upload.png" style="display: block; margin:auto;" title="Ziehen Sie zum Hochladen Dateien über dieses Feld oder klicken Sie!" class="dz-message">';html+='</form>';html+='<div id="kimb_enc_outer">';html+='<input type="checkbox" id="kimb_enc_oo"> Dateien verschlüsseln <sup>BETA</sup>';html+='<div id="kimb_enc_div" style="display:none;">';html+='<input type="password" id="kimb_enc_passw" placeholder="Passwort"><br />';html+='<small><input type="checkbox" id="kimb_enc_klar"> Klartext</small><br />';html+='<small>Geben Sie ein Passwort zu Verschlüsselung der Dateien an.</small><br />';html+='<small>Alle verschlüsselten Dateien erhalten die Endung <code>.kimb_enc</code> und können nur mit Passwort heruntergeladen werden.</small>';html+='</div>';html+='</div>';j_alert(html,600);if(typeof InstallTrigger==='undefined'&&typeof window.chrome==='undefined'){$("div#kimb_enc_div").html("Für die Verschlüsselung wird Mozilla Firefox oder Google Chrome benötigt! Die Entschlüsselung funktioniert aber auch in anderen Browsern.");$("div#kimb_enc_div").css("color","red")}
$("input#kimb_enc_oo").change(function(){if($("input#kimb_enc_oo:checked").length==1){$("div#kimb_enc_div").css("display","block")}
