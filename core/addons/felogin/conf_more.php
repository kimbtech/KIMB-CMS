...
if( isset( $_GET['gruppe'] ) && !isset( $_GET['del'] ) ){
	...
	$sitecontent->add_site_content('<tr> <td><form method="post" action="'.$addonurl.'"><input type="text" name="newgr" placeholder="hinzufügen" ></td> <td>'.id_dropdown( 'firstrequid', 'siteid' ).'</td> <td><input type="submit" value="Los"></form></td> </tr>');
...
if( !empty( $_POST['selfoo'] ) && !empty( $_POST['areaoo'] ) && !empty( $_POST['selfgr'] ) && !empty( $_POST['akzep'] ) && !empty( $_POST['requid'] ) ){
...
$sitecontent->add_site_content('<input name="selfgr" value="'.$feconf->read_kimb_one( 'selfreggruppe' ).'" ><span style="display:inline-block;" title="Bitte geben Sie an zu welcher Gruppe selbstständig registrierte User hinzugefügt werden sollen." class="ui-icon ui-icon-info"></span><br />');
