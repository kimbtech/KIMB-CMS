...
if( isset( $_POST['onoff'] ) ){
	...
	if( $_POST['onoff'] == 'on' || $_POST['onoff'] == 'off' ){
		...
		if( $_POST['onoff'] == 'on' && $felogin['file']->read_kimb_one( 'fe' ) != 'all' ){
			$felogin['file']->write_kimb_replace( 'fe' , 'all' );
			$sitecontent->echo_message( 'Ausgaben aktiviert!' );
		}
		elseif( $_POST['onoff'] == 'off' && $felogin['file']->read_kimb_one( 'fe' ) == 'all' ){
			$felogin['file']->write_kimb_replace( 'fe' , 'none' );
		...
		foreach( $allteile as $teil ){	
			$dings = $felogin['cont']->read_kimb_one( $teil );
			if( $dings == '' && $_POST[$teil] != '' ){
				$felogin['cont']->write_kimb_new( $teil , $_POST[$teil] );
				$sitecontent->echo_message( '"'.$teil.'" hinzugefügt!' );
			}
			elseif( $dings != $_POST[$teil]  && $_POST[$teil] != '' ){
				$felogin['cont']->write_kimb_replace( $teil , $_POST[$teil] );
				$sitecontent->echo_message( '"'.$teil.'" geändert!' );
			}
			elseif( $_POST[$teil] == '' ){
				$felogin['cont']->write_kimb_delete( $teil );
...
if( $felogin['file']->read_kimb_one( 'fe' ) == '' ){
	$felogin['file']->write_kimb_new( 'fe' , 'all' );
}

if( $felogin['file']->read_kimb_one( 'fe' ) != 'all' ){
	$off = ' checked="checked" ';
	$on = ' ';
}
else{
...
$sitecontent->add_site_content('<textarea name="sitefi" id="nicedit1" style="width:99%; height:100px;" >'.$felogin['cont']->read_kimb_one( 'sitefi' ).'</textarea> (Zusätzlicher Seiteninhalt oben &uarr; )<br />');

$sitecontent->add_site_content('<textarea name="sitese" id="nicedit2" style="width:99%; height:100px;" >'.$felogin['cont']->read_kimb_one( 'sitese' ).'</textarea> (Zusätzlicher Seiteninhalt unten &uarr; )<br />');

$sitecontent->add_site_content('<textarea name="addonfi" id="nicedit3" style="width:99%; height:100px;" >'.$felogin['cont']->read_kimb_one( 'addonfi' ).'</textarea> (Zusätzliche Add-onausgabe oben &uarr; )<br />');

$sitecontent->add_site_content('<textarea name="addonse" id="nicedit4" style="width:99%; height:100px;" >'.$felogin['cont']->read_kimb_one( 'addonse' ).'</textarea> (Zusätzliche Add-onausgabe unten &uarr; )<br />');

$sitecontent->add_site_content('<textarea name="header" style="width:99%; height:100px;" >'.$felogin['cont']->read_kimb_one( 'header' ).'</textarea> (Zusätzlicher Header &uarr; )<br />');

$sitecontent->add_site_content('<input name="title" style="width:60%;" value="'.$felogin['cont']->read_kimb_one( 'title' ).'" > (Allgemeiner Titel)<br />');
