...
$eaconffile = new KIMBdbf( 'addon/kimbea__conf.kimb' );

//Add-on URL, damit kann man später einfacher abrbeiten
$addonurl = $allgsysconf['siteurl'].'/kimb-cms-backend/addon_conf.php?todo=more&addon=kimbea';
...
foreach( $ibinfo as $key => $val ){
	//leer
	if( empty( $val ) ){
		//Beispiel setzen
		$ibinfo[$key] = $examples['001'][$key];
	}
}
...
foreach( $trackinfo as $key => $val ){
	...
	if( empty( $val ) ){
		//Beispiel setzen
		$trackinfo[$key] = $examples['001'][$key];
	}
}
